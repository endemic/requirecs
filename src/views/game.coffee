...
], ($, Backbone, levels, env, CGRect, Input, Scene, DialogBox, template) ->
	class GameScene extends Scene
		...
		onActionEnd: (e) ->
			...
			if @rectangle.css('display') is 'block'
				...
				@elem.append clone
				clone.data 'area', area # For the win condition check
		...
		resize: (width, height, orientation) ->
			...
			if orientation is 'landscape'
				gridWidth = Math.round(height * 0.95 / 10) * 10 	# 90%
				@grid.width(gridWidth)
				@grid.height(gridWidth)
				# Manually apply some margin to the grid
				@grid.css 'margin', "#{(height - gridWidth) / 2}px #{width - gridWidth - (width * 0.05)}px 0"
			else if orientation is 'portrait'
				gridWidth = Math.round(width * 0.95 / 10) * 10 	# 90%
				@grid.width(gridWidth)
				@grid.height(gridWidth)
				@grid.css 'margin', '5% auto 0 auto'
